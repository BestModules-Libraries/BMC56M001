...
uint8_t DataBuff[14] = {1,2,3,4,5,6,7,8,9,0x0A,0x0B,0x0C,0x0D,0x0E};
...
void setup()
{
  ...
  BMC56.setDeviceRole(Peer);          //set device role
}
...
void RFMessage_Process()
{
  ...
  { 
   ...
   case 0x02:   
      if(Flag_Pairing)
      {
        BMC56.writePairPackage(Pair_Op_Code,14,DataBuff);                //Send request pairing packets during pair mode                 
      } 
      if(Flag_PairSuccess)
      {
